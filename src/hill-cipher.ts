...
function stringToTwoDimArray(s: string, rows: number, cols: number, template: string) {
  const arr = twoDimArrayFactory(rows, cols, 0);
  for (let r = 0, position = 0; r < rows; ++r) {
    for (let c = 0; c < cols; ++c, ++position) {
      const char = s[position] || template[0];
...
export default class HillCipher {
  ...
  constructor(option: IHillCipherConstructOption) {
    ...
    const size = ~~Math.sqrt(keyLength);
    const keyArr = stringToTwoDimArray(option.key, size, size, template);
